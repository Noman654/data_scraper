...
def get_download_links(item_url, format='PDF'):
    response = requests.get(item_url, verify=False)  # Disable SSL verification
    soup = BeautifulSoup(response.content, 'html.parser')
    download_links = []
    for a in soup.select('.row a[href]'):
        if format in a['href']:
            download_links.append(a['href'])
    return download_links

async def download_file(session, url, output_folder, item_name, download_link, collection):
    ...
    for attempt in range(retry_attempts):
        try:
            async with session.get(url, timeout=300, ssl=False) as response:  # Disable SSL verification
            ...
            if attempt == retry_attempts - 1:
                ...
                try:
                    response = requests.get(url, stream=True, timeout=300, verify=False)  # Disable SSL verification
                    response.raise_for_status()
                    with open(local_path, 'wb') as f:
...
async def process_item_async(item_url):
    ...
    base_url = "https://www.sanskrit.nic.in/"
    ...
    async with aiohttp.ClientSession() as session:
        for link in download_links:
            file_path = await download_file(session, base_url + link, output_folder, item_name, link, collection)
            if file_path:
...
def main():
    item_urls = [
        "https://www.sanskrit.nic.in/ebooks.php"
